RailsPulse.configure do |config|
  ...
  config.authentication_enabled = true #Rails.env.production?

  # Where to redirect unauthorized users
  config.authentication_redirect_path = "/admin"

  # Custom authentication method - choose one of the examples below:
  config.authentication_method = proc {
    session_id = cookies.signed[:session_id]
    unless session_id && Session.find_by(id: session_id)
      redirect_to main_app.new_session_path, alert: "Please log in"
    end
  }
